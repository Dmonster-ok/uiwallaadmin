...
const fetchData = async () => {
    isLoading = true;
    try {
        const response = await axios.get('http://localhost:3000/component?page=' + page);
...
<template>
    ...
    <div class="flex flex-row overflow-y-scroll justify-center">
        <button class="material-icons absolute p-4 top-[50%] left-0" @click="prev">arrow_back_ios</button>
        <div class="h-full mt-1 p-2 flex flex-wrap gap-4 justify-center">
                        ...
                        <transition enter-active-class="transition ease-out duration-100"
                            ...
                            <MenuItems class="menu-items absolute">
                                <div>
                                    ...
                                    <button class="hover:text-red-600 hover:transition"
                                        @click="updateComponent(component._id)">
                                        Update
                                    </button>
                                    </MenuItem>
                                    <MenuItem class="menu-item hover:text-red-600 hover:transition" v-slot="{ active }">
                                    <button class="hover:text-red-600 hover:transition"
                                        @click="deleteComponent(component._id)">
                                        Delete
                                    </button>
                                    </MenuItem>
                                </div>
        ...
        <button class="material-icons absolute p-4 top-[50%] right-4 " @click="next">arrow_forward_ios</button>
    </div>
</template>
